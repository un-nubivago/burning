    ...
    public void onCopperCube(GameTestHelper game) {
        ...
        for (var pos : set) {
            ...
            if (pos.equals(targetPos)) {
                game.assertValueEqual(1600L, storage.getAmount(), literal("target storage amount"));
            } else {
                game.assertValueEqual(800L, storage.getAmount(), literal("storage amount"));
            }
        }
        ...
        while (result.size() < 10) {
            ...
            if (result.add(game.absolutePos(pos))) {
                game.assertBlock(pos, Blocks.AIR::equals,
                        block -> literal("Block should be air, its " + game.getBlockState(pos).getBlock()));
                ...
                try (var transaction = Transaction.openOuter()) {
                    ...
                    game.assertValueEqual(base, storage.insert(FuelVariant.COAL, base, transaction),
                            literal("inserted amount"));
